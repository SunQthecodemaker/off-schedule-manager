...
async function renderManagerLeaveList() {
    ...
    try {
        ...
        // ✅ 매니저 탭에서 승인/반려 함수 재정의 (자동 갱신 포함)
        setupManagerApprovalHandlers();
...
// ✅ 매니저 탭 승인/반려 핸들러 설정 (자동 갱신 포함)
function setupManagerApprovalHandlers() {
    // 기존 window 함수 백업
    const originalMiddleApproval = window.handleMiddleApproval;
    const originalFinalApproval = window.handleFinalApproval;
    ...
    window.handleMiddleApproval = async function(requestId, status) {
        ...
        try {
            ...
            // ✅ 매니저 탭 갱신 (관리자 모드가 아닌 경우)
            if (state.employee?.activeTab === 'leaveList') {
                await renderManagerLeaveList();
            } else {
                // 관리자 모드인 경우 기존 동작
                if (window.loadAndRenderManagement) {
