import { state, db, isVisibleIn } from './state.js?v=20260819b';
import { _, _all, show, hide } from './utils.js';
import { getLeaveDetails, isLeaveInPeriod, getPartTimeHolidayLeaveDates } from './leave-utils.js?v=20260819b';
import { stageChange, isStagingMode, shouldStage, notifyStaged, approvePendingChange, rejectPendingChange } from './staging.js?v=20260819b';
...
export function getLeaveListHTML() {
    ...
    const monthSections = buildLeaveMonthSectionsHTML(currentMonthVal);
    ...
    return `
        ...
        <div id="settlement-modal" class="modal-overlay hidden">
            <div class="modal-content">
                ...
                <div id="settlement-modal-body" class="space-y-4"></div>
            </div>
        </div>
    `;
}
...
export function getLeaveStatusHTML() {
    ...
    const employeeLeaveData = validEmployees.map(emp => {
        ...
        // adjustments JSONB에서 해당 주기의 조정값 반영
        const periodKey = pStart.format('YYYY-MM-DD');
        const periodAdjs = (emp.adjustments || {})[periodKey];
        if (periodAdjs && offset !== 0) {
            leaveDetails.adjustment = periodAdjs.adjustment || 0;
            leaveDetails.carriedOverCnt = periodAdjs.carried || 0;
            leaveDetails.final = leaveDetails.legal + leaveDetails.adjustment + leaveDetails.carriedOverCnt;
        }
        ...
        if (lastYearAdjs) {
            lastYearRawDetails.adjustment = lastYearAdjs.adjustment || 0;
            lastYearRawDetails.carriedOverCnt = lastYearAdjs.carried || 0;
            lastYearRawDetails.final = lastYearRawDetails.legal + lastYearRawDetails.adjustment + lastYearRawDetails.carriedOverCnt;
        }
        const lastYearDetails = lastYearRawDetails;
    ...
    return `
        <style>
            ...
            /* 반차 대각선 카드: 오전=좌상 삼각형, 오후=우하 삼각형. 날짜는 각 삼각형에 작게 */
            .leave-box.split {
                position: relative;
                padding: 0;
                overflow: hidden;
            ...
            .leave-box.split .half-am,
            .leave-box.split .half-pm {
                position: absolute;
                font-size: 8px;
                line-height: 1;
                font-weight: bold;
...
function getLeaveStatusRow(emp) {
    ...
    // 사용 엔트리 = 반차도 각각 한 칸. 오전반차 2번을 "1일 연차" 한 칸으로 합치지 않는다.
    // (숫자 사용량 usedDays 는 반차 0.5 합산이지만, 그리드는 점유한 날짜 수만큼 칸을 그린다)
    const usedEntries = emp.usedDates || [];
    ...
    const bucketFor = (dayPos) => {
        ...
        if (dayPos < finalLeaves) return 'regular'; // 음수 조정 등 잔여 구간
        return 'borrowed'; // 한도 초과 = 당겨쓰기
    };

    // 1) 사용 칸 구성
    //    - 종일 연차 = 꽉 찬 카드 1개 (날짜 중앙)
    //    - 반차 = 한 카드를 대각선으로 나눠 오전(좌상)·오후(우하)를 합쳐 표기. 각 반차 날짜를 작게 따로.
    //    - 오전+오후 짝이 다 차면 1일 완성(양쪽 채움), 한쪽만 있으면 반쪽만 채운 '미완성' 카드.
    ...
    // 카드 목록: 당겨쓰기 작년분(개별) 먼저, 그 다음 올해분(날짜순)
    const cards = [];
    ...
    cards.forEach(card => {
        ...
        if (card.kind === 'full') {
            ...
            const amSpan = am
                ? `<span class="half-am" data-request-id="${am.requestId || ''}" data-type="${am.type || 'formal'}" title="오전반차: ${am.date}">${dayjs(am.date).format('M.D')}</span>`
                : `<span class="half-am empty-half" title="오전반차 미사용">·</span>`;
            const pmSpan = pm
                ? `<span class="half-pm" data-request-id="${pm.requestId || ''}" data-type="${pm.type || 'formal'}" title="오후반차: ${pm.date}">${dayjs(pm.date).format('M.D')}</span>`
                : `<span class="half-pm empty-half" title="오후반차 미사용">·</span>`;
            const titleParts = [];
...
async function handleLeaveBoxClick(e) {
    // 반차 대각선 카드는 오전/오후 각 삼각형(span)에 개별 data-request-id 를 가짐 → 그걸 우선
    const idEl = e.target.closest('[data-request-id]');
    if (!idEl) return;
    // 빈칸/미사용 칸(요청 id 없음)은 무시
    const requestId = idEl.dataset.requestId;
    if (!requestId) return;
...
window.viewLeaveApplication = function (requestId) {
    ...
    const isHalfApp = request.leave_type === 'am_half' || request.leave_type === 'pm_half';
    const halfAppLabel = request.leave_type === 'am_half' ? ' (오전반차)' : request.leave_type === 'pm_half' ? ' (오후반차)' : '';
    const leaveDates = (request.dates || []).join(', ') + halfAppLabel;
    const daysCount = (request.dates?.length || 0) * (isHalfApp ? 0.5 : 1);
...
async function handleLeaveBoxDblClick(e) {
    ...
    // 이미 사용된 칸(종일 used / 반차 대각선 split)은 수동 등록 대상 아님
    if (box.classList.contains('used') || box.classList.contains('split')) return;
